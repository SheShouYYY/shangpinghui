          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="item in spuSaleAttrList" :key="item.id">
                ...
                <!-- 点击事件 判断给谁active 传入每个数组和数组里面的对象 -->
                <dd
                  changepirce="40"
...
export default {
  ...
  // 改变商品参数选择的active
  methods: {
    // value是每个对象，arr是对象外包裹的数组
    changeActive(value, arr) {
      // 先把每个数组里面的isChecked变成0
      arr.forEach((element) => {
        element.isChecked = '0'
      })
      // 把选中的改成1
    ...
    changeSkuNum() {
      // 乘以1判断是不是数字
      const result = this.skuNum * 1
      // 获取第0项
      const result2 = this.skuNum.split('')[0]
      // 如果不是数字或者小于1
    ...
    async addShopCar() {
      try {
        // 调用dispach获取请求,判断有没有异常
        const result = await this.$store.dispatch('Detail/addShopCarSuccess', {
          // 传入需要的参数
